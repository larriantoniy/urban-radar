...
func TestRuntimeMigrationProtectsCurrentInvariants(t *testing.T) {
	...
	for _, required := range []string{"source_checkpoints", "news_check_runs", "content_drafts", "human_review_required", "human_review_notes", "approved_content_hash", "approved_by", "rejected_by", "review_notified_at", "review_notification_external_id", "first_seen_at", "last_seen_at", "processing_state", "READY_TO_PUBLISH", "retry_stage", "runtime_data", "research_rounds BETWEEN 0 AND 1", "UNIQUE (source, source_item_id)", "source_fingerprint", "INTERRUPTED", "COMPLETED"} {
